/* =====================================================================
   Procedure: dbo.SP_Subjects_GetForStaff_Group0_WithNationalId
   Kind: RETRIEVE
   Purpose: Get distinct subjects for staff where Staff_Material.groupid = 0 (with NatianalId)
            NOTE: AcademicYear is INT in Staff_Material
   Author: Eslam Ragab
   Version: 1.0.1
   CreatedOn: 2025-12-26
   ===================================================================== */
IF OBJECT_ID('dbo.SP_Subjects_GetForStaff_Group0_WithNationalId', 'P') IS NOT NULL
    DROP PROCEDURE dbo.SP_Subjects_GetForStaff_Group0_WithNationalId;
GO

CREATE PROCEDURE dbo.SP_Subjects_GetForStaff_Group0_WithNationalId
    @p_faculty_code   INT,
    @p_department_id  INT,
    @p_phase_id       INT,
...
EXEC dbo.SP_Subjects_GetForStaff_Group0_WithNationalId
    @p_faculty_code=12,
    @p_department_id=5,
    @p_phase_id=2,
    @p_national_id=N'12345678901234',
    @p_class_id=1,
    @p_academic_year=2025,   -- ✅ مثال رقم (غيّره للقيمة الموجودة عندك)
    @p_lang='ar',
