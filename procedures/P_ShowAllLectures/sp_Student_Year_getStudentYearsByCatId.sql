/* =====================================================================
   Procedure: dbo.SP_GetStudentYearsByCatId
   Kind: RETRIEVE
   Purpose: Retrieve active student years by category (Page ShowAllLectures)
   Ticket: LEC-005
   Author: Osama Mahmoud
   Version: 1.0.0
   CreatedOn: 2025-12-24
   ===================================================================== */
IF OBJECT_ID('dbo.SP_GetStudentYearsByCatId', 'P') IS NOT NULL
    DROP PROCEDURE dbo.SP_GetStudentYearsByCatId;
GO

CREATE PROCEDURE dbo.SP_GetStudentYearsByCatId
    @p_cat_id       INT,
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            SET @o_message = CASE WHEN LEFT(@p_lang,2)='ar' 
                                  THEN N'لا توجد سنوات دراسية لهذه الفئة' 
                                  ELSE N'No student years found for this category' END;
            RETURN;
        END

        SET @o_success_code = 0;
        SET @o_message = CASE WHEN LEFT(@p_lang,2)='ar' 
                              THEN N'تم جلب السنوات بنجاح' 
                              ELSE N'Success' END;
    END TRY
    BEGIN CATCH
...
EXEC dbo.SP_GetStudentYearsByCatId
    @p_cat_id = 10,
    @p_lang = 'ar',
